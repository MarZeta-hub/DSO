...
/*
 ...
 /*  ret = closeFileIntegrity(ret);
    if (ret == -1 || ret == -2 || ret == -3)
	{
		fprintf(stdout, "%s%s%s%s%s", ANSI_COLOR_BLUE, "TEST closeFileOIntegrity ", ANSI_COLOR_RED, "FAILED\n", ANSI_COLOR_RESET);
		return -1;
	}
...
*/
    ...
    if (ret != 0)
	{
		fprintf(stdout, "%s%s%s%s%s", ANSI_COLOR_BLUE, "TESTcheckFile ", ANSI_COLOR_RED, "FAILED\n", ANSI_COLOR_RESET);
		return -1;
	}
...
/*
	...
	int writte = writeFile(0, hola, strlen(hola));
	if (writte == -1)
	{
